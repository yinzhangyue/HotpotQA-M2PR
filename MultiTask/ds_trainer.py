...
from fastNLP.core.sampler import RandomSampler, ConstTokenNumSampler
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if metric_key is not None:
            ...
            self.metric_key = (
                metric_key[1:]
                if metric_key[0] == "+" or metric_key[0] == "-"
                else metric_key
            )
        else:
        ...
        self.pin_memory = kwargs.get(
            "pin_memory",
            False if parse_version(torch.__version__) == parse_version("1.9") else True,
        )
        if isinstance(model, nn.parallel.DistributedDataParallel):  # 如果是分布式的
            ...
            if isinstance(sampler, ConstTokenNumSampler):  # 直接使用固定token数量的Sampler
                assert isinstance(train_data, DataSet), (
                    f"When sampler is `ConstTokenNumSampler`, the train_data must"
                    f" be `DataSet`."
                )
                sampler(train_data)
    ...
    def train(self, load_best_model=True, on_exception="auto", **kwargs):
        ...
        try:
            ...
            if (
                ...
                and self.best_dev_perf is not None
                and load_best_model
            ):
                model_name = "best_" + "_".join(
                    [self.model.__class__.__name__, self.metric_key, self.start_time]
                )
    ...
    def _train(self):
        ...
        with inner_tqdm(
            ...
            for epoch in range(self.epoch, self.n_epochs + 1):
                ...
                for batch_x, batch_y in self.data_iterator:
                    ...
                    if (
                        ...
                        eval_str = (
                            "Evaluation on dev at Epoch {}/{}. Step:{}/{}: ".format(
            ...
            if self.dev_data is not None and (
                self.validate_every > 0 and self.n_steps % self.validate_every != 0
            ):
                eval_res = self._do_validation(epoch=epoch, step=self.step)
                eval_str = "Evaluation on dev at Epoch {}/{}. Step:{}/{}: ".format(
                    epoch, self.n_epochs, self.step, self.n_steps
...
def _get_value_info(_dict):
    ...
    for key, value in _dict.items():
        ...
        if isinstance(value, torch.Tensor):
            _str += "\t{}: (1)type:torch.Tensor (2)dtype:{}, (3)shape:{} ".format(
                key, value.dtype, value.size()
            )
...
def _check_code(
    ...
    if dev_data is not None:
        tester = Tester(
            data=dev_data[: batch_size * DEFAULT_CHECK_NUM_BATCH],
            model=model,
            metrics=metrics,
            batch_size=batch_size,
            verbose=-1,
        ...
        _check_eval_results(
            metrics=evaluate_results, metric_key=metric_key, metric_list=metrics
        )
    ...
    if isinstance(metrics, dict):
        ...
        if metric_key is None:
            indicator_val, indicator = (
                list(metric_dict.values())[0],
                list(metric_dict.keys())[0],
            )
        else:
            # metric_key is set
