...
class HotpotPipe(Pipe):
    ...
    def process(self, data_bundle: DataBundle) -> DataBundle:
        ...
        def document_tokenize(document):
            ...
            doc_length = [len(i) for i in self.tokenizer(document, truncation=True, max_length=self.max_length)["input_ids"]]
        ...
        def cal_DOC1_SEP_num(input_ids):
            SEP_num = 0
            DOC_num = 0
            length_input_ids = len(input_ids)
            for i in range(length_input_ids):
                SEP_num += 1 if input_ids[i] == self.SEP_id else 0
                DOC_num += 1 if input_ids[i] == self.DOC_id else 0
                if DOC_num == 2:
                    break
            return SEP_num

        data_bundle.apply_field(cal_DOC1_SEP_num, field_name="input_ids", new_field_name="DOC1_SEP_num")
...
class HotpotQATestPipe(Pipe):
    ...
    def process(self, data_bundle: DataBundle) -> DataBundle:
        ...
        def change_input_ids(instance):
            input_ids = ""
            for gold_doc in instance["gold_doc_pair"]:
                input_ids += instance["docs"][gold_doc]
            tokenized_gold_pair = self.tokenizer(
                input_ids,
                truncation=True,
                padding=True,
                max_length=self.max_length,
            )
            return {"input_ids": tokenized_gold_pair["input_ids"], "token_type_ids": tokenized_gold_pair["token_type_ids"], "attention_mask": tokenized_gold_pair["attention_mask"]}

        data_bundle.apply_more(change_input_ids)
