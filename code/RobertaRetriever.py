...
########## RobertaRetriever ##########
class RobertaRetriever(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        for b in range(batch_size):
            ...
            stage_one_outputs = self.roberta(input_ids=question_document_ids, attention_mask=question_document_attention_mask, return_dict=True)
            ...
            if gold_doc_pair is not None:
                ...
                rank_loss = rank_loss_function(document_probability.squeeze(), document_labels)
                total_rank_loss = total_rank_loss + rank_loss

            # Select document pair
            question_double_document_ids_length = torch.tensor([question_length[b] + doc_length[b][seleced_documents[0]] + doc_length[b][seleced_documents[1]], question_length[b] + doc_length[b][seleced_documents[0]] + doc_length[b][seleced_documents[2]], question_length[b] + doc_length[b][seleced_documents[1]] + doc_length[b][seleced_documents[2]]], device=device, dtype=torch.long)
            ...
            stage_two_outputs = self.roberta(input_ids=question_double_document_ids, attention_mask=question_double_document_attention_mask, return_dict=True)
            ...
            if gold_doc_pair is not None:
                ...
                document_pair_loss = torch.mean(torch.sum(-torch.log(double_document_probability.squeeze().softmax(dim=-1)) * double_document_label.squeeze(), dim=-1))
